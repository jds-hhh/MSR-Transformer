...
class Build_Tgt_Features():
    def __init__(self, model,fields,opt,shard_pairs):
        self.model = model
        self.fields = fields
        self.opt=opt
        ...
        self.low_freq=self.opt.low_freq
        self.chunk_size=self.opt.chunk_size
        self.decoder_quantizer = TorchPQCodec(index=faiss.read_index(self.opt.quantizer_path))
        if self.opt.gpu!=-1:
            self.decoder_quantizer.cuda(self.opt.gpu)

        for i, (src_shard, tgt_shard) in enumerate(shard_pairs):
            src_data = {"reader": self.src_reader, "data": src_shard}
            tgt_data = {"reader": self.tgt_reader, "data": tgt_shard}
        ...
        self.data_iter = inputters.OrderedIterator(
            ...
            batch_size_fn=max_tok_len if self.opt.batch_type == "tokens" else None,
            train=False,
            sort=False,
        ...
        self.total_tgt_tokens = self.tgt_len_offsets[-1].item()  # 求得src的总tokens数目
        ...
        self.quantize_hidden_size=self.decoder_quantizer.centroids.shape[0]
    ...
    def load_token_2d_offsets(self,data_dir, mode,freq,max_sent=0):
        """
        ...
        for sent_idx in tqdm(range(max_sent), desc="Gathering token offsets"):
            ...
            for intra_offset, token in enumerate(src_ids):
                token_idx=self._tgt_vocab.stoi[token]
    ...
    def get_freq(self):
        freq = [0 for i in range(len(self._tgt_vocab.itos))]
        for example in tqdm(self.data.examples, desc='calculate freqs'):
            for t in example.tgt[0]:
                freq[self._tgt_vocab.stoi[t]]+=1
        return freq

    def build_token_dstores(self,data_dir, subset="train", lang="C",
        ...
        freq_file = os.path.join(data_dir, subset + '.' + lang  + '.freq')
        ...
        max_sent = len(self.data.examples)
        offset_end = min(offset_end, self.total_tgt_tokens) if offset_end else self.total_tgt_tokens
        token_2d_offsets = self.load_token_2d_offsets(data_dir, subset,freq,max_sent)
        assert all(x.shape[0] == f for x, f in zip(token_2d_offsets, freq)), \
            "offsets shape should be consistent with frequency counts"

        tgt_mmap_file = os.path.join(data_dir, f"{subset}-features", f"all.mmap.decoder")
        #warmup_mmap_file(tgt_mmap_file, verbose=False)
        tgt_mmap_features = np.memmap(tgt_mmap_file, dtype=np.float32, mode='r',
                                      shape=(self.total_tgt_tokens, self.hidden_size))

        token_end = min(token_end, len(src_dict)) if token_end else len(src_dict)
        pbar = tqdm(total=self.total_tgt_tokens, desc="Building DecoderStore for each token")
        ...
        if self.opt.overwrite:
            print("\nremove decoder feature")
            for token_idx in range(token_start, token_end): #先删除所有decoder_key.npy与decoder_val.npy数据
                dstore_dir = os.path.join(data_dir,
                                          self.opt.dstore_dir,
                                          f"token_{token_idx}")
                DecoderStore.remove(dstore_dir)
        ...
        def run(features, o_start, o_end):
            ...
            def build_dstore(token_idx):
                ...
                positions = self.tgt_len_offsets[sent_ids] + token_offsets
                ...
                mode = "r+" if DecoderStore.exists(dstore_dir) else "w+"
                #高频词给予chunk_size存储空间，低频词给予low_freq存储空间
                dstore_size=self.low_freq if freq[token_idx]<self.low_freq else self.chunk_size
                datastore = DecoderStore(dstore_size=dstore_size, vocab_size=0,
                                      hidden_size=self.quantize_hidden_size,
                                      dstore_dir=dstore_dir,
                                      mode=mode,
                                      freq=freq[token_idx])
                total = positions.shape[0]
                s = 0
                while s < total and dstore_offset+s<self.chunk_size:        #s既要小于总数，又不能超过限定的chunk_size
                    end = min(s + batch_size, total)
                    if dstore_offset+end>self.chunk_size:   #防止存储的数据超过chunk_size
                        end=self.chunk_size-dstore_offset
                    # try:
                    batch_positions = positions[s: end]
                    batch_sent_ids = sent_ids[s: end]
                    batch_token_offsets=token_offsets[s:end]
                    batch_features=features[batch_positions - o_start]
                    batch_features=torch.from_numpy(batch_features)
                    if self.opt.gpu!=-1:
                        batch_features=batch_features.cuda(self.opt.gpu)
                    batch_features=self.decoder_quantizer.encode(batch_features)     #量化特征
                    batch_features=batch_features.cpu().numpy()
                    datastore.keys[dstore_offset + s: dstore_offset + end, :] = batch_features      #将量化特征进行存储
                    tokens=np.zeros((len(batch_token_offsets),1),dtype=np.int32)
                    for i,(sid,offset) in enumerate(zip(batch_sent_ids,batch_token_offsets)):
                         tokens[i]=self._tgt_vocab.stoi[self.data.examples[sid].tgt[0][offset]]
                    datastore.vals[dstore_offset + s: dstore_offset + end]=tokens
                    datastore.sent_ids[dstore_offset + s: dstore_offset + end]=batch_sent_ids
                    #pbar.update(end - s)
                    s = end
                    # except:
                    #     print('dstore excpetion')
                pbar.update(total)
...
def main(opt):
    ...
    build_tgt_features = Build_Tgt_Features(model, fields, opt,shard_pairs)
    build_tgt_features.build_token_dstores(data_dir=opt.data_dir, lang=opt.lang, subset=opt.mode,
...
def _get_parser():
    parser = ArgumentParser(description='build tgt features.py')
    opts.config_opts(parser)
    opts.translate_opts(parser)
    opts.build_tgt_features_opt(parser)
    return parser
