...
def translate_with_knn_opts(parser):
    group = parser.add_argument_group('Translate with knn')
    group.add('--train_src', '-train_src', required=True,
              help="Trainset source")
    group.add('--train_tgt', '-train_tgt', required=True,
              help="Trainset target")
    group.add('--max_neighbors', '-max_neighbors', type=int,default=512,
              help="Loads the maximum number of nearest neighbors from the testset")
    group.add('--data_path',"-data_path", type=str, required=True, help="Path to binary dataset directory")
    group.add('-no_memory', "-no_memory",action='store_true', help="Data is not loaded into memory")
    group.add("-source_lang", type=str, default="P", help="Source language to find neighbors")
    group.add("-target_lang", type=str, default="C", help="Target language to find neighbors")
    group.add("-gen_subset", type=str, default='test',choices=['train','valid','test'],help=" data subset to generate (train, valid, test)")
    group.add("-global_neighbor", type=bool, default=False, help="Whether to use a global neighbor")
    group.add('-neighbor_metric', type=str, default="l2", choices=["l2", "ip", "cosine"],
              help='faiss index metric, l2 for L2 distance, ip for inner product, '
                   'cosine for cosine similarity')
    group.add("-quantize", action='store_true', help="Loading quantize features")
    group.add("-quantizer_path", type=str, default='', help="Path of quantized feature")
    group.add("-link_ratio", type=float, default=0.0,
              help="ratio of vocab probs predicted by edge linking")

    group.add("-link_temperature", type=float, default=1.0,
              help="temperature used by edge linking")
    group.add("-topk", type=int, default=-1,
              help="use topk-scored neighbor tgt nodes for link prediction and probability compuation."
                   "-1 means using every node.")
    group.add("-sim_metric", type=str, default="l2", choices=["l2", "ip", "cosine", "biaf"],
              help="metric for distance")
    group.add("-multiproc", action='store_true', help="Using multiple processes")


def translate_trace_opts(parser):
    group = parser.add_argument_group('Translate trace')
    group.add('--max_neighbors', '-max_neighbors', type=int,default=512,
              help="Loads the maximum number of nearest neighbors from the testset")
    group.add("-data_dir", type=str, required=True, help="path to binary dataset directory")
    group.add("-dstore_dir", type=str, required=False, default='train_P_data_stores',
    ...
    group.add('-neighbor_metric', type=str, default="l2", choices=["l2", "ip", "cosine"],
              help='faiss index metric, l2 for L2 distance, ip for inner product, '
                   'cosine for cosine similarity')
    group.add('-use_sent_ids', action='store_true',
              help='use the sentence id to record which sentence the target token belongs to')
    group.add("-quantize", action='store_true', help="Loading quantize features")
    group.add("-quantizer_path", type=str, default='', help="Path of quantized feature")
    group.add("-link_ratio", type=float, default=0.0,
              help="ratio of vocab probs predicted by edge linking")
    group.add("-link_temperature", type=float, default=1.0,
              help="temperature used by edge linking")
    group.add("-threshold", type=float, default=0.8,
              help="Threshold of similarity between the feature and its neighbor")
    group.add("-nprobe", type=int, default=64, help="nprobe of faiss")
    group.add("-topk", type=int, default=-1,
              help="use topk-scored neighbor tgt nodes for link prediction and probability compuation."
                   "-1 means using every node.")
    group.add("-sim_metric", type=str, default="l2", choices=["l2", "ip", "cosine", "biaf"],
              help="metric for distance")
    group.add("-multiproc", action='store_true', help="Using multiple processes")
    group.add("-cache_size",type=int, default=128, help="Cache size")
    group.add("-load_pretext",action='store_true', help="load pretext")
    group.add('--pretext_src', '-pretext_src', required=False,default='',
              type=str,help="preprocessing the source of the text")
    group.add('--pretext_tgt', '-pretext_tgt', required=False,default='',
              type=str,help="preprocessing the target of the text")
    group.add('--save_update', '-save_update',action='store_true',
              help="save data updates")
    group.add('--save_dir', '-save_dir', default='', type=str,
              help="the directory where the data store is stored")
    group.add('--init_remove', '-init_remove', action='store_true',
              help="Remove all stored data")


def store_update_opts(parser):
    group = parser.add_argument_group('store update')
    group.add('--max_neighbors', '-max_neighbors', type=int, default=512,
              help="Loads the maximum number of nearest neighbors from the testset")
    group.add("-data_dir", type=str, required=True, help="path to binary dataset directory")
    group.add("-dstore_dir", type=str, required=False, default='train_P_data_stores',
    ...
    group.add("-sim_metric", type=str, default="l2", choices=["l2", "ip", "cosine", "biaf"],
              help="metric for distance")
    group.add("-cache_size", type=int, default=128, help="Buffer size")
    group.add("-load_pretext", action='store_true', help="load pretext")
    group.add('--save_update', '-save_update', action='store_true',
              help="save data updates")
    group.add('--save_dir', '-save_dir', default='',type=str,
...
def search_opts(parser):
    ...
    group.add("-dstore_dir", type=str, required=False, default='train_P_data_stores',
              help="path to datastore directory")
    group.add('--src_text', '-src_text', type=str, required=True, help="the text used to find the source")
    group.add('--tgt_text', '-tgt_text', type=str, required=True, help="the text used to find the target")
    group.add('-neighbor_metric', type=str, default="l2", choices=["l2", "ip", "cosine"],
              help='faiss index metric, l2 for L2 distance, ip for inner product, '
                   'cosine for cosine similarity')
    group.add("-quantize", action='store_true', help="Loading quantize features")
    ...
    group.add('--init_remove', '-init_remove',action='store_true',
              help="Remove all stored data")
    group.add('--update', '-update', action='store_true', help="update data")
    group.add('-use_sent_ids', action='store_true',
              help='use the sentence id to record which sentence the target token belongs to')



def build_ds_opts(parser):
    group = parser.add_argument_group('Build ds')
    group.add("-store_encoder", action="store_true", default=False,
                        help="store encoder feature")
    group.add("-store_decoder", action="store_true", default=False,
                        help="store decoder feature")
    group.add("-start_batch", type=int, default=0,
                        help="start batch idx")
    group.add("-end_batch", type=int, default=0,
                        help="end batch idx")
    group.add("-suffix", type=str, default="",
                        help="suffix of extracted decoder features")
    group.add("-gen_subset", type=str, default="",
              help="the subset of the generated data")
    group.add("-data_dir", type=str, required=True, help="path to binary dataset directory")
    group.add("-prefix", type=str, default="de-en", help="prefix of binary file")
    group.add("-lang", type=str, default="P", help="source_lang")
    group.add("-mode", type=str, default="train", help="train/valid/test")
    group.add("-workers", type=int, default=1, help="num workers")
    group.add("-start", type=int, default=0, help="start token idx")
    group.add("-end", type=int, default=0, help="end token idx")
    group.add("-offset_start", type=int, default=0, help="start token offset")
    group.add("-offset_end", type=int, default=0, help="end token offset")
    group.add("-offset_chunk", type=int, default=0,
                        help="max token offset span. If set, split chunks from offset_start to offset_end")
    group.add("-use_memory", action="store_true", default=False, help="load feature mmap to memory")
    group.add("-max_sent", type=int, default=0,
                        help="if specified, use at most max_sent sentences for datastores")
    group.add("-all_in_one", default=False, action="store_true", help="if set, build a data_store"
                                                                                 "that contain all tokens")
    group.add("-regenerate_labels", default=False, action="store_true",
                        help="change ds vals from sent_idx and token_idx to labels.")


def index_build_opt(parser):
    group = parser.add_argument_group('Index build')
    group.add("-dstore_dir", type=str, required=True, help="paths to data store. if provided multiple,"
                                                                      "use ',' as separator")
    group.add("-save_dir", type=str, required=False,default='', help="save path to binary dataset directory")
    group.add("-subdirs", action="store_true",
                        help="if True, each subdir of dstore_dir is a candidate dstore dir.")
    group.add("-subdirs_range", type=str, default="",
    ...
    group.add("-index_type", type=str, default="auto",
                        help="faiss index type"),
    group.add('-seed', type=int, default=0, help='random seed')
    group.add('-chunk_size', default=1000000, type=int,
                        help='can only load a certain amount of data to memory at a time.')
    group.add("-low_freq", type=int, default=30000, help="Low frequency word threshold")
    group.add('-use_gpu', default=False, action='store_true',
                        help='if true, use gpu for training')
    group.add('-use_chunk', default=False, action='store_true',
              help='can only load a certain amount of data to store')
    group.add('-workers', type=int, default=1, help='number of cpu')
    group.add('-metric', type=str, default="l2", choices=["l2", "ip", "cosine"],
                        help='faiss index metric, l2 for L2 distance, ip for inner product, '
                             'cosine for cosine similarity')
    group.add("-use_memory", action="store_true",
...
def find_knn_opt(parser):
    group = parser.add_argument_group('Find knn neighbors')
    group.add("-data_dir", type=str, required=True, help="path to binary dataset directory")
    group.add("-prefix", type=str, default="de-en", help="prefix of binary file")
    group.add("-mode", type=str, default="test", help="train/valid/test")
    group.add("-neighbor_subset", type=str, default="train",
                        help="train/valid/test, from which subset to get neighbors")
    group.add("-lang", type=str, default="de", help="language to find neighbors")
    group.add("-pretrained_file", type=str, default="", help="if some token's neighbor have been calculated")
    group.add("-pretrained_num", type=int, default=0,
                        help="use rows in pretrained neighbor file from 0 to num")
    group.add("-use-gpu", action="store_true", help="use gpu")
    group.add("-workers", type=int, default=1, help="number of threads")
    group.add("-k", type=int, default=5, help="number of neighbors")
    group.add("-nprobe", type=int, default=64, help="nprobe of faiss")
    group.add('-metric', type=str, default="l2", choices=["l2", "ip", "cosine"],
                        help='faiss index metric, l2 for L2 distance, ip for inner product, '
                             'cosine for cosine similarity')
    group.add("-global_neighbor", default=False, action="store_true",
                        help="if set, find knn neighbor without the constraint that token_idx must be same.")
    group.add("-max_sent", type=int, default=0,
                        help="if specified, use at most max_sent sentences for datastores")
    group.add("-use_memory", default=False, action="store_true",
                        help="if specified, load features to memory")
    group.add("-offset_start", type=int, default=0, help="start token offset")
    group.add("-offset_end", type=int, default=0, help="end token offset")
    group.add("-offset_chunk", type=int, default=0,
...
def quantize_features_opt(parser):
    group = parser.add_argument_group('Quantize features')
    group.add("-data_dir", type=str, required=True, help="path to binary dataset directory")
    group.add("-prefix", type=str, default="de-en", help="prefix of binary file")
    group.add("-index", type=str, default="OPQ128_512,PQ128", help="quantizer index")
    group.add("-lang", type=str, default="P", help="source_lang")
    group.add("-subset", type=str, default="train", help="train/valid/test")
    group.add("-code_size", type=int, default=128, help="bytes of quantized feature")
    group.add("-chunk_size", type=int, default=10000000, help="maximum number of features to train")
    group.add("-compute_error", action="store_true", default=False,
                        help="compute reconstruction error")
    group.add("-suffix", type=str, default="", help="feature suffix")

def build_tgt_features_opt(parser):
    group = parser.add_argument_group('Build tgt features')
    group.add("-start_batch", type=int, default=0,
              help="start batch idx")
    group.add("-end_batch", type=int, default=0,
              help="end batch idx")
    ...
    group.add("-gen_subset", type=str, default="",
              help="the subset of the generated data")
    group.add("-data_dir", type=str, required=True, help="path to binary dataset directory")
    group.add("-dstore_dir", type=str, required=False,default='train_P_data_stores', help="path to datastore directory")
    group.add("-mode", type=str, default="train", help="train/valid/test")
    group.add("-workers", type=int, default=1, help="num workers")
    group.add("-start", type=int, default=0, help="start token idx")
    group.add("-end", type=int, default=0, help="end token idx")
    ...
    group.add("-max_sent", type=int, default=0,
              help="if specified, use at most max_sent sentences for datastores")
    group.add("-quantizer_path", type=str, default='', help="Path of quantized feature")
    group.add("-chunk_size", type=int, default=200000, help="maximum number of features to train")
    group.add("-low_freq", type=int, default=30000, help="Low frequency word threshold")
    group.add("-lang", type=str, default="P", help="source_lang")
    group.add("-overwrite", action="store_true",
              help="if True, delete old decoder_store files before generating new ones")
