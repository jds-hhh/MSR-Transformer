...
import faiss
from onmt.knn.word_tracker import WordTracker
...
class Inference(object):
    ...
    def translate(
        ...
        #加载预文本
        if self.opt.load_pretext:
            src_shards = split_corpus(self.opt.pretext_src, self.opt.shard_size)
            tgt_shards = split_corpus(self.opt.pretext_tgt, self.opt.shard_size)
            ...
            # 预加载文本数据集
            self.pretext_dataset = inputters.Dataset(
                self.fields,
            ...
            self.pretext_dataset=None

        if self.opt.link_ratio>0:
            self.word_tracker=WordTracker(opt=self.opt,fields=self.fields,pretext_dataset=self.pretext_dataset,
                                          batch_size=batch_size,model=self.model)
        ...
        for batch in tqdm(data_iter,desc='translate with trace'):
...
class Translator(Inference):
    ...
    def _translate_batch_with_strategy(
        ...
        if self.opt.link_ratio > 0:
            if self.opt.multiproc:  #使用多进程
                ntgt_feats, ntgt_labels = self.word_tracker.multipro_find_token_neighbors_features(batch, enc_out)
            else:
                ntgt_feats, ntgt_labels=self.word_tracker.find_token_neighbors_features(batch,enc_out)
        ...
        for step in range(decode_strategy.max_length):
            ...
            if self.opt.link_ratio > 0:
                log_probs = self.word_tracker.get_knn_scores(batch,dec_out,step,log_probs.exp(),ntgt_feats,ntgt_labels)
        ...
        #更新数据，先获取原先的数据
        if self.opt.link_ratio > 0 and batch.tgt!=None:
            self.model.decoder.init_state(src, enc_out, enc_states)
            tgt = batch.tgt
            dec_in = tgt[:-1]
            true_dec_out, attns = self.model.decoder(dec_in, enc_out,
                                                         memory_lengths=src_lengths, with_align=False)
            ...
            # true_dec_out=true_dec_out.squeeze(1)
            src = srcs[:, 0, 0].tolist()
            tgt = tgts[1:-1, 0, 0].tolist()  # 去掉tgt的<s>与</s>
            self.word_tracker.update(src,tgt,enc_out,true_dec_out,batch.indices)
