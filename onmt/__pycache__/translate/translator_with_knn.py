...
from onmt.mmap_dataset import MmapDataset
from tqdm import tqdm
import json,math
from onmt.utils.misc import split_corpus
from onmt.pq_wrapper import TorchPQCodec
import faiss
from onmt.knn.knn_nmt_dataset import KNNNMTDataset
...
class Inference(object):
    ...
    def token_freq_path(self,data_dir, mode, lang, max_sent=0):
        return os.path.join(data_dir, f"{mode}{'_'+str(max_sent)+'sent' if max_sent else ''}.{lang}.freq")

    def token_offsets_path(self, data_dir, mode, lang):
        return os.path.join(data_dir, f"{mode}.{lang}.2d_offsets.npy")

    def token_neighbor_path(self,data_dir, mode, lang, k, metric="l2", global_neighbor=False):
        return os.path.join(data_dir,
                            f"{mode}.{lang}.token_neighbors_{metric}{'_all' if global_neighbor else ''}.mmap.{k}")

    def feature_path(self,data_dir, mode, type="encoder", suffix=""):
        assert type in ["encoder", "decoder"]
        return os.path.join(data_dir, f"{mode}-features", f"all.mmap.{type}{suffix}")

    def quantized_feature_path(self,data_dir, mode, type="encoder", suffix=""):
        assert type in ["encoder", "decoder"]
        return os.path.join(data_dir, f"{mode}-features", f"quantized-feature-{type}{suffix}.new.npy")


    def cal_dataSize(self,examples):
        src_size = []
        tgt_size = []
    ...
    def load_dataset(self,data):
        ...
        src_token_freq = eval(open(self.token_freq_path(data_dir=self.opt.data_path,
                                                 mode="train",       #这里应该是train的，现在只是为了测试程序
                                                 lang=src),'r',encoding='utf-8').read())

        #测试集
        pair_dataset = data

        src_shards = split_corpus(self.opt.train_src,self.opt.shard_size)
        tgt_shards = split_corpus(self.opt.train_tgt, self.opt.shard_size)

        src_shard=None
        tgt_shard=None
        for src_data,tgt_data in zip(src_shards,tgt_shards):
            src_shard=src_data
            tgt_shard=tgt_data

        src_data = {"reader": self.src_reader, "data": src_shard, "features": {}}
        tgt_data = {"reader": self.tgt_reader, "data": tgt_shard, "features": {}}
        train_readers, train_data = inputters.Dataset.config(
            [("src", src_data), ("tgt", tgt_data)]
        )

        #训练集
        train_dataset = inputters.Dataset(
            self.fields,
        ...
        max_neighbors = self.opt.max_neighbors
        if max_neighbors:
            neighbor_file = self.token_neighbor_path(data_dir=self.opt.data_path, mode=self.opt.gen_subset,
                                                lang=self.opt.source_lang, k=max_neighbors,
                                                metric=self.opt.sim_metric,
                                                global_neighbor=self.opt.global_neighbor)       #从测试集中加载最近邻居
            if os.path.exists(neighbor_file):
                print(f"Use neighbor file {neighbor_file}")
            else:
                raise FileNotFoundError(f"Could not find neighbor file for max_neighbors of {self.opt.gen_subset} data "
                                        f"from {self.opt.max_neighbors} to {max_neighbors}")

        src_dataSize,_ = self.cal_dataSize(pair_dataset.examples)
        src_len_offsets = np.cumsum(src_dataSize)
        src_len_offsets = np.insert(src_len_offsets, 0, 0)
        num_src_tokens=src_len_offsets[-1]

        src_len_offsets_file = self.token_len_offsets_path(self.opt.data_path, 'train', 'src')
        if os.path.exists(src_len_offsets_file):
            nsrc_sent_offsets = np.load(src_len_offsets_file, allow_pickle=True)
            num_nsrc_tokens = nsrc_sent_offsets[-1]
        else:
            raise FileNotFoundError(f"Could not find {src_len_offsets_file} file")
        tgt_len_offsets_file = self.token_len_offsets_path(self.opt.data_path, 'train', 'tgt')
        if os.path.exists(tgt_len_offsets_file):
            ntgt_sent_offsets = np.load(tgt_len_offsets_file, allow_pickle=True)
            num_ntgt_tokens = ntgt_sent_offsets[-1]
        else:
            raise FileNotFoundError(f"Could not find {tgt_len_offsets_file} file")


        # nsrc_dataSize,ntgt_dataSize=self.cal_dataSize(train_dataset.examples)      #训练集tokens总数
        # nsrc_sent_offsets=np.cumsum(nsrc_dataSize)
        # nsrc_sent_offsets=np.insert(nsrc_sent_offsets,0,0)
        # ntgt_sent_offsets=np.cumsum(ntgt_dataSize)
        # ntgt_sent_offsets = np.insert(ntgt_sent_offsets, 0, 0)
        #
        # train_offsets_path=self.token_offsets_path(data_dir=self.opt.data_path,mode='train',lang=self.opt.source_lang)
        # if os.path.exists(train_offsets_path):
        #     print('Use train offsets file')
        #     ntgt_sent_offsets = np.load(train_offsets_path,allow_pickle=True)
        # else:
        #     raise FileNotFoundError(f"Could not {train_offsets_path} file" )
        #
        # num_ntgt_tokens=ntgt_sent_offsets[-1]

        neighbors=MmapDataset(neighbor_file,dtype=np.int64,shape=(num_src_tokens,max_neighbors,2),warmup=warmup)

        self.feat_path2array={}

        if memory:
            print(f"loading data into memory: {neighbor_file}")
            neighbors = PlasmaArray(np.array(neighbors._mmap))
            self.feat_path2array[neighbor_file]=neighbors

        hidden_size = json.load(open(self.feature_path(data_dir=self.opt.data_path, mode="train", type="encoder") + ".json"))[
            "hidden_size"]
        decoder_quantizer=None
        if self.opt.quantize:
            quantize_feature_file = self.quantized_feature_path(data_dir=self.opt.data_path, mode="train",
                                                           type="decoder", suffix='')
            print(f"loading ntgt feature into memory: {quantize_feature_file}")
            neighbor_tgt_feats = PlasmaArray(np.load(quantize_feature_file))
            if os.path.exists(neighbor_file):
                print("Use decoder quantizer")
                decoder_quantizer = TorchPQCodec(index=faiss.read_index(self.opt.quantizer_path))
            else:
                raise FileNotFoundError("Could not find decoder quantizer}")

        else:
            ntgt_feat_file = self.feature_path(data_dir=self.opt.data_path, mode="train", type="decoder",
                                          suffix='')
            if warmup:
            ...
            if memory:
                if ntgt_feat_file in self.feat_path2array:
                    ...
                    print(f"loading data into memory: {ntgt_feat_file}")
                    neighbor_tgt_feats = PlasmaArray(np.array(neighbor_tgt_feats._mmap))
        ...
        gnmt_dataset = KNNNMTDataset(
            ...
            src_len_offsets=src_len_offsets,
            token_neighbors=neighbors,
            neighbor_tgt_feature=neighbor_tgt_feats,
            max_neighbors_per_token=self.opt.max_neighbors,
            train_dataset=train_dataset,
            nsrc_sent_offsets=nsrc_sent_offsets,
            ntgt_sent_offsets=ntgt_sent_offsets,
            decoder_quantizer=decoder_quantizer,
            src_token_freq=src_token_freq,
            sort_key=inputters.str2sortdict[self.data_type],
        )
    ...
    def translate(
        ...
        self.gnmt_dataset=self.load_dataset(data)
        ...
        for batch in tqdm(data_iter,desc='translate with knn'):
    ...
    def _decode_and_generate(
        ...
        #改写
        #self.gnmt_dataset.quantizer_decode()
...
class Translator(Inference):
    ...
    def _translate_batch_with_strategy(
        ...
        if self.opt.multiproc:
            if self.opt.link_ratio>0.0:
                print('using multiprocess')
                ntgt_feats, ntgt_labels=self.gnmt_dataset.get_token_neighbors_features(batch,self.model.decoder.embeddings.word_vec_size)
        ...
        for step in range(decode_strategy.max_length):
            ...
            if self.opt.multiproc:
                    if self.opt.link_ratio > 0.0:
                        log_probs = self.gnmt_dataset.get_knn_scores(batch,step,dec_out,log_probs.exp(),ntgt_feats,ntgt_labels)
            else:
                log_probs = self.gnmt_dataset.knn_scores(batch, step, dec_out, log_probs.exp())
