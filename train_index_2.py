...
def get_auto_index_type(dstore_size,chunk_size,hidden_size,opt):
    ...
    if dstore_size <= 3000 and chunk_size>3000:
        return "IDMap,,Flat"
    clusters = min(int(4 * math.sqrt(dstore_size)), dstore_size // 30)
    if dstore_size <= opt.low_freq:
        return "IDMap,,Flat"
    if dstore_size<30000 and dstore_size >= opt.low_freq:
        clusters = 64
    if dstore_size < 10 ** 6:
...
def build(dstore_dir,opt):
    ...
    index_type = get_auto_index_type(dstore_size, max_num, hidden_size,opt)
